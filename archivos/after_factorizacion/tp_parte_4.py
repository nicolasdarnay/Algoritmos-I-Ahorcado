...
diccionario = tp_parte_2.obtener_diccionario()


#maxima longitud de palabra en el diccionario
sacar_maxima = lambda diccionario: len(max(list(diccionario.keys()), key = len))


def solicitud_de_ingreso():
    """Pregunta al usuario si quiere ingresar una longitud específica
    antes de jugar. Autores: Roy Fiorilo/Ignacio Chacón"""
...
def validar_ingreso(longitud, diccionario):
    ...
    Autores: Roy Fiorilo/Ignacio Chacón"""
    longitud_maxima = sacar_maxima(diccionario)
    rv = longitud
